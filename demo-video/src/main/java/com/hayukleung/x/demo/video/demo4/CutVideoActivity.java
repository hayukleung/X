...
public class CutVideoActivity extends BaseActivity implements View.OnClickListener {
  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
    ...
    dp50 = (int) getResources().getDimension(R.dimen.xp20_0);
    ...
    vv_play.addOnLayoutChangeListener(new View.OnLayoutChangeListener() {
      ...
      public void onLayoutChange(View v, int left, int top, int right, int bottom, int oldLeft,
        ...
        cv_video.setMargin(vv_play.getLeft(), vv_play.getTop(), windowWidth - vv_play.getRight(),
            windowHeight - vv_play.getBottom() - dp50);
      }
    });
  }
  ...
  private String cutVideo(String path, int cropWidth, int cropHeight, int x, int y) {
    ...
    String outPut = VideoApplication.VIDEO_PATH + "/cutVideo.mp4";

    //./ffmpeg -i 2x.mp4 -filter_complex "[0:v]setpts=0.5*PTS[v];[0:a]atempo=2.0[a]" -map "[v]" -map "[a]" output3.mp4
    String filter =
        String.format(Locale.getDefault(), "crop=%d:%d:%d:%d", cropWidth, cropHeight, x, y);
