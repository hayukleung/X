...
public class RecordActivity extends BaseActivity implements View.OnClickListener {
  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
    ...
    rb_start = (RecordButton) findViewById(R.id.rb_start);
    ...
    rb_start.setOnGestureListener(new RecordButton.OnGestureListener() {
  ...
  private void syntVideo() {
    ...
    sb.append(" -y");
    String output = VideoApplication.VIDEO_PATH + "/finish.mp4";
    sb.append(" " + output);
  ...
  @Override public void onClick(View v) {
    switch (v.getId()) {
      case R.id.iv_back:
        if (rb_start.isDeleteMode()) {
          // 判断是否要删除视频段落
          MediaObject.MediaPart lastPart =
              mMediaObject.getPart(mMediaObject.getMediaParts().size() - 1);
          mMediaObject.removePart(lastPart, true);
          rb_start.setProgress(mMediaObject.getDuration());
          rb_start.deleteSplit();
          changeButton(mMediaObject.getMediaParts().size() > 0);
          iv_back.setImageResource(R.drawable.video_delete);
        } else if (mMediaObject.getMediaParts().size() > 0) {
          rb_start.setDeleteMode(true);
          iv_back.setImageResource(R.drawable.video_delete_click);
        }
        break;
      case R.id.iv_finish:
        videoFinish();
      ...
      case R.id.iv_next:
        ...
        intent.putExtra("path", VideoApplication.VIDEO_PATH + "/finish.mp4");
