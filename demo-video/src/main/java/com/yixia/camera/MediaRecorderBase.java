...
public abstract class MediaRecorderBase implements Callback, PreviewCallback, IMediaRecorder {
   ...
   * @param sh
   */
  @SuppressWarnings("deprecation") public void setSurfaceHolder(SurfaceHolder sh) {
    if (sh != null) {
      sh.addCallback(this);
  ...
  @SuppressWarnings("deprecation") protected void prepareCameraParaments() {
    ...
    for (int x = 0; x < mSupportedPreviewSizes.size(); x++) {
      ...
      if (size.width * size.height
          == MediaRecorderBase.VIDEO_WIDTH * MediaRecorderBase.VIDEO_HEIGHT) {
        flag = true;
      }
    }
  ...
  protected void setPreviewCallback() {
    ...
    if (size != null) {
      ...
      Log.e("Yixia", "startPreview...setPreviewCallbackWithBuffer...width:"
          + size.width
          + " height:"
          + size.height);
    } else {
  ...
  public boolean autoFocus(AutoFocusCallback cb) {
    if (camera != null) {
      try {
        ...
        if (e != null) Log.e("Yixia", "autoFocus", e);
      }
    }
    return false;
  }
  ...
  public boolean manualFocus(AutoFocusCallback cb, List<Area> focusAreas) {
    if (camera != null && focusAreas != null && mParameters != null && DeviceUtils.hasICS()) {
      try {
        ...
        if (mParameters.getMaxNumMeteringAreas() > 0) mParameters.setMeteringAreas(focusAreas);
  ...
  public void prepare() {
    mPrepared = true;
    if (mSurfaceCreated) startPreview();
  }
  ...
  @Override public void onAudioError(int what, String message) {
    if (mOnErrorListener != null) mOnErrorListener.onAudioError(what, message);
  }

  /** 接收音频数据 */
  @Override public void receiveAudioData(byte[] sampleBuffer, int len) {
  ...
  @Override public void surfaceCreated(SurfaceHolder holder) {
    ...
    if (mPrepared && !mStartPreview) startPreview();
  }
  ...
  public void startEncoding() {
    if (mMediaObject == null || mEncodeHanlder == null) return;
    ...
    new AsyncTask<Void, Void, Boolean>() {
      ...
      @Override protected Boolean doInBackground(Void... params) {
        ...
        String cmd = String.format(
            "ffmpeg -i \"%s\" -vcodec copy -acodec copy -absf aac_adtstoasc -f mp4 -movflags faststart \"%s\"",
            mMediaObject.getConcatYUV(), mMediaObject.getOutputTempVideoPath());
  ...
  public static class EncodeHandler extends Handler {
    ...
    @Override public void handleMessage(Message msg) {
      ...
      if (mrb == null || mrb.mOnEncodeListener == null) return;
      ...
      switch (msg.what) {
        ...
        case MESSAGE_ENCODE_PROGRESS:
          //查询片段是否都已经转码完成
          final int progress =
              UtilityAdapter.FilterParserAction("", UtilityAdapter.PARSERACTION_PROGRESS);
          if (progress == 100) {
