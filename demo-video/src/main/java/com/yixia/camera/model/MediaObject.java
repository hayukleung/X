...
@SuppressWarnings("serial") public class MediaObject implements Serializable {
  ...
  public void removePart(MediaPart part, boolean deleteFile) {
    if (mMediaList != null) mMediaList.remove(part);
  ...
  public int getCurrentIndex() {
    ...
    if (part != null) return part.index;
    return 0;
  }

  /** 获取当前分块 */
  public MediaPart getCurrentPart() {
    if (mCurrentPart != null) return mCurrentPart;
    if (mMediaList != null && mMediaList.size() > 0) {
  ...
  public MediaPart getPart(int index) {
    if (mCurrentPart != null && index < mMediaList.size()) return mMediaList.get(index);
    return null;
  }
  ...
  public static class MediaPart implements Serializable {
    ...
    public void writeAudioData(byte[] buffer) throws IOException {
      if (mCurrentOutputAudio != null) mCurrentOutputAudio.write(buffer);
    }

    /** 写入视频数据 */
    public void writeVideoData(byte[] buffer) throws IOException {
      if (mCurrentOutputVideo != null) mCurrentOutputVideo.write(buffer);
    }
