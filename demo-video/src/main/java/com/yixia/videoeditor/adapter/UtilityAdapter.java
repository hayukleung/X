...
public class UtilityAdapter {
   ...
   * 特效处理器
   *
   * @param settings 特效设置: inv=/sdcard/v.rgb; ina=/sdcard/p.pcm; out=/sdcard/o.mp4;
   * text=/sdcard/txt.png
   ...
   */
  public static native int SoundEffect(String inPath, String outPath, float tempoChange,
      float pitch, int pitchSemitone);

  /** 底层音频初始化 */
  @SuppressWarnings("deprecation") public static boolean ndkAudioInit() {
    int desiredFrames = AudioTrack.getMinBufferSize(44100, AudioFormat.CHANNEL_CONFIGURATION_MONO,
        AudioFormat.ENCODING_PCM_16BIT);// * 8;
    //desiredFrames = 101376
    if (mAudioTrack == null) {
      mAudioTrack =
          new AudioTrack(AudioManager.STREAM_MUSIC, 44100, AudioFormat.CHANNEL_CONFIGURATION_MONO,
              AudioFormat.ENCODING_PCM_16BIT, desiredFrames, AudioTrack.MODE_STREAM);
